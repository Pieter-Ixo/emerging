...
> {
  return request<INewsPostsResponse>(
    `https://ixoworld.ghost.io/ghost/api/v3/content/posts?key=${API_KEY}&limit=${1}&fields=title,feature_image,published_at,url&filter=tag:emerging`
  );
}
export async function requestNewsPosts(
  ...
  return request<INewsPostsResponseExtended>(
    `https://ixoworld.ghost.io/ghost/api/v3/content/posts?key=${API_KEY}&limit=${limit}&page=${page}&fields=title,feature_image,published_at,excerpt,id,url&filter=tag:emerging`
  );
}
