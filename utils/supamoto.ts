...
export const roundNumber = (num: number, scale: number) => {
  if (!("" + num).includes("e")) {
    ...
    return +(
      Math.round(
        (+arr[0] + "e" + sig + (+arr[1] + scale)) as unknown as number
      ) +
      "e-" +
      scale
    );
...
export const hoursSaved = (totalPelletsPurchased: number) =>
  roundNumber(totalPelletsPurchased * 0.5, 4);

export const lifeYearsSaved = (totalPelletsPurchased: number) =>
  roundNumber(totalPelletsPurchased * 0.0001, 4);
...
export const formatSessions = (
  ...
  ses.forEach((s, i) => {
    if (i === 0) return (prevSession = s);
      ...
      if (s.time === prevSession.time)
        return (prevSession = {
          ...prevSession,
          value: prevSession.value + s.value,
        });
      else {
        finalSessions.push({
          ...prevSession,
          value: valueFormatter(prevSession.value),
        });
        prevSession = s;
      }
    }
...
export const formatPellets = (
  ...
  ses.forEach((s, i) => {
    if (i === 0) return (prevPellet = s);
      ...
      if (s.time === prevPellet.time)
        return (prevPellet = {
          ...prevPellet,
          value: prevPellet.value + s.value,
        });
      else {
        finalPellets.push({
          ...prevPellet,
          value: valueFormatter(prevPellet.value),
        });
        prevPellet = s;
      }
    }
