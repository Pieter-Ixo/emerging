...
export const queryAllBalances = async (
  ...
  try {
    const response = await queryClient.cosmos.bank.v1beta1.allBalances({
      address,
    });
...
export const queryDelegatorDelegations = async (
  ...
  try {
    ...
    const delegatorDelegations: DELEGATION[] = delegationResponses.map(
      (delegation: DelegationResponse) => ({
        ...
        balance: {
          denom:
            delegation?.balance?.denom ?? stakeCurrency.coinMinimalDenom ?? "",
...
export const queryDelegationTotalRewards = async (
  ...
  try {
    ...
    const totalRewards = response.total.find(
      (total) => total.denom === stakeCurrency.coinMinimalDenom
    );
    const delegationTotalRewards = {
      total: totalRewards
        ? {
            amount: totalRewards.amount.slice(
              0,
      ...
      rewards: response.rewards.map((reward) => {
        const delegationReward = reward.reward.find(
          (reward) => reward.denom === stakeCurrency.coinMinimalDenom
        );
        ...
        return {
          ...
          rewards: delegationReward
            ? {
                amount: delegationReward.amount.slice(
                  0,
                  delegationReward.amount.length - 18
                ),
...
export const queryValidators = async (queryClient: QUERY_CLIENT) => {
  try {
    ...
    let newValidatorList: VALIDATOR[] = validators.map(
      (validator: Validator) => {
        const validatorVotingPower = (
          (Number(validator.tokens) /
            Math.pow(10, 6) /
            (totalTokens / Math.pow(10, 6))) *
          100
        ).toFixed(2);
        ...
        return {
          ...
          commission:
            Number(validator.commission?.commissionRates?.rate ?? 0) /
            Math.pow(10, 16),
    ...
    newValidatorList = filterValidators(
      newValidatorList,
      FILTERS.VOTING_POWER_RANKING as VALIDATOR_FILTER_TYPE,
      ""
    );
    newValidatorList = newValidatorList.map(
