      ...
      asset
        ? {
            ...
            undelegating:
              asset.undelegating +
              sumArray(
                unbondingDelegation.entries.map((x) =>
                  Number(x.balance.amount ?? 0)
                )
              ),
          }
        : {
            ...
            available: 0,
            staked: 0,
            undelegating: sumArray(
              unbondingDelegation.entries.map((x) => Number(x.balance))
            ),
            token: {
              amount: "0",
              denom: unbondingDelegation.entries[0].balance?.denom,
...
export const broadCastMessages = async (
  ...
  const feeDenom = getFeeDenom(
    suggestedFeeDenom,
    chain.feeCurrencies as TOKEN_ASSET[]
  );
  switch (wallet.walletType) {
    case WALLET_TYPE.keplr:
      return await keplrBroadCastMessage(
        msgs,
        memo,
        fee,
        feeDenom,
    ...
    case WALLET_TYPE.opera:
      return await operaBroadCastMessage(
        msgs,
        memo,
        fee,
        feeDenom,
    ...
    case WALLET_TYPE.walletConnect:
      return await WCBroadCastMessage(
        msgs,
        memo,
        fee,
        feeDenom,
