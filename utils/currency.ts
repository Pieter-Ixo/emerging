...
export const microAmountToAmount = (
  microAmount: number,
  microUnits: number = 6
) => {
...
export const formattedAmountToNumber = (amount: string) =>
...
export type TokenDropdownType = ArrayElement<
  ReturnType<typeof generateUserTokensDropdown>
>;
...
export const validateAmountAgainstBalance = (
  amount: number,
  balance: number,
  balanceMicroUnits: boolean = true
) => {
  const realBalance = balanceMicroUnits ? balance / 10 ** 6 : balance;
  return amount <= realBalance;
};

export const getFeeDenom = (
  suggestedDenom: string = "",
  feeCurrencies: TOKEN_ASSET[]
): string => {
  if (suggestedDenom) {
    const suggestedTokenAsset =
      customQueries.currency.findTokenFromDenom(suggestedDenom);
    if (
      suggestedTokenAsset?.isFeeCurrency &&
      feeCurrencies.some((currency) => currency.coinDenom === suggestedDenom)
    )
      return suggestedTokenAsset.coinMinimalDenom;
  }
  const feeCurrency = feeCurrencies.find((cur: TOKEN_ASSET) => {
    const curTokenAsset = customQueries.currency.findTokenFromDenom(
...
export const getDisplayDenomFromDenom = (
  denom: string,
...
export const getMicroUnitsFromDenom = (
  denom: string,
  tokenAsset?: TokenAsset
): number => {
  const token = tokenAsset ?? customQueries.currency.findTokenFromDenom(denom);
...
export const getDisplayDenomFromCurrencyToken = (
  currencyToken?: CURRENCY_TOKEN
) => currencyToken?.token?.coinDenom ?? currencyToken?.denom ?? "";

export const getDenomFromCurrencyToken = (currencyToken?: CURRENCY_TOKEN) =>
  currencyToken?.denom ?? "";

export const getAmountFromCurrencyToken = (currencyToken?: CURRENCY_TOKEN) =>
  Number(currencyToken?.amount ?? 0);

export const getDecimalsFromCurrencyToken = (currencyToken?: CURRENCY_TOKEN) =>
  Number(currencyToken?.token?.coinDecimals ?? 0);

export const getIbcStatusFromCurrencyToken = (currencyToken?: CURRENCY_TOKEN) =>
  currencyToken?.ibc ?? false;

export const getCoinImageUrlFromCurrencyToken = (
  currencyToken?: CURRENCY_TOKEN
) => currencyToken?.token?.coinImageUrl;

export const getTokenTypeFromCurrencyToken = (
  currencyToken?: CURRENCY_TOKEN,
  chainName?: string
