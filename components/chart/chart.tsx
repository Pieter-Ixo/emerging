...
const Chart = ({ id, lineColor = '#000', dataFormatter, dataType = STOVE_DATA_TYPES.cooking_sessions }: ChartProps) => {
	...
	useEffect(() => {
		...
		const newSeries = chart.addAreaSeries({ lineColor, lineWidth: 3, topColor: '#ffff', bottomColor: '#fff1' });
	...
	const handlePeriodChange = (event: React.MouseEvent<HTMLButtonElement>, period: STOVE_PERIODS) => {
		...
		<div className={styles.chartContainer}>
			...
			<div className={styles.buttons}>
				{Object.values(STOVE_PERIODS).map(p => (
					<button key={p} onClick={e => handlePeriodChange(e, p)} className={p === period ? styles.selected : undefined}>
						{p}
					</button>
				))}
			</div>
